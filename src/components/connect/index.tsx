...
export const Connect = () => {
  ...
  const login = useCallback(async (signature: string) => {
    try {
      const res = await fetch(`${API_URL}/auth/sign`, {
        ...
        body: JSON.stringify({ address: account.address?.toLowerCase(), signature  }),
      })
      if (res.ok) {
  ...
  const createSignature = useCallback(async ({ nonce }) => {
    try {
